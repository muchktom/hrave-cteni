...
// Helper to filter words based on available letters
export function getAvailableWords(allowedLetters: string[]): string[] {
  const allowedSet = new Set(allowedLetters.map(l => l.toUpperCase()));
  
  return EASY_WORDS.filter(word => {
